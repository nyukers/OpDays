-- Dva HOSTNAME s odnim IP-adr
prompt ------------------------------------------

prompt _ за двома станціями зафіксовано одну ІР-адресу (host_ip2.sql)

prompt ------------------------------------------

col hostname format a20

select ss.IP, ss.HOSTNAME 
from host_ip_adres ss
where ss.IP in(
 select s.ip
 from host_ip_adres s
 group by s.ip
  having count(s.hostname) > 1
 );
/*
...
'' SIG '' AAIBAAIBAAIBAAIBADAgMAwGCCqGSIb3DQIFBQAEEIJO
'' SIG '' OkRUGq2j/kJaVZ3ki8+gggGrMIIBpzCCAVGgAwIBAgIQ
...
'' SIG '' AYI3AgEVMB8GCSqGSIb3DQEJBDESBBCdwZPTUXHjhkAI
'' SIG '' ZzUTd+HNMA0GCSqGSIb3DQEBAQUABECBJGBFgIInBd1v
'' SIG '' 4bWaDzapMOhGWDHe2i0cyCJKJeVlklHmMGGD6DGAa7l/
'' SIG '' OPJmyK2nrlTv0M3qYfgKPMoETI02
