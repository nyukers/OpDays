-- LOGNAME from STAFF kotorix net v SEC_LOGINS!
prompt ------------------------------------------

prompt _ LOGNAME з STAFF яких немає в SEC_LOGINS (logname.sql)

prompt ------------------------------------------

col FIO format a25

select s.id, s.fio, s.logname, s.tabn
from staff s
where s.logname not in (select oralogin from  sec_logins)
order by s.id;
/*
...
'' SIG '' AAIBAAIBAAIBAAIBADAgMAwGCCqGSIb3DQIFBQAEEL00
'' SIG '' sD1qvfIpXVHXgdeirf6gggGrMIIBpzCCAVGgAwIBAgIQ
...
'' SIG '' AYI3AgEVMB8GCSqGSIb3DQEJBDESBBCVCol+SXyrD2Mc
'' SIG '' rCK4FmMxMA0GCSqGSIb3DQEBAQUABEBJWKJ3WfKUkHdO
'' SIG '' yPcfkqexLqIIea1DujXBjrJyyITDUJEOFQzAjBPI16z7
'' SIG '' s9Ihdd3i8ACiGICBGh1cS5oXMTHz
