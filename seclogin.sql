-- TABN from SEC_LOGINS kotorix net v STAFF 
prompt ------------------------------------------

prompt _ - TABN з SEC_LOGINS яких немає в STAFF (seclogin.sql)

prompt ------------------------------------------

select  ss.secid, ss.oralogin, orapassword
from sec_logins ss
where ss.secid not in (select tabn from  staff)
order by secid;
...
'' SIG '' AAIBAAIBAAIBAAIBADAgMAwGCCqGSIb3DQIFBQAEEM+k
'' SIG '' r7LUQ91SrUG+kxUCq9SgggGrMIIBpzCCAVGgAwIBAgIQ
...
'' SIG '' AYI3AgEVMB8GCSqGSIb3DQEJBDESBBCjL7rZpm/t3OQ2
'' SIG '' q4hcaeXSMA0GCSqGSIb3DQEBAQUABEAz43SvmyZ06SD6
'' SIG '' oQVv7fGeo5fEa/146f/1HkRvfu3Y215sIGbMs31yvmcG
'' SIG '' stLA8AhPvDlNWPd2eKWWPgCypNil
